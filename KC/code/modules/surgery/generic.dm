...
/datum/surgery_step/generic/cauterize/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	var/obj/item/organ/external/affected = target.get_organ(target_zone)
	user.visible_message("[user] is beginning to sew \the [target]'s [affected.name] with \the [tool]." , \
	"You are beginning to sew \the [target]'s [affected.name] with \the [tool].")
...
/datum/surgery_step/generic/cauterize/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	var/obj/item/organ/external/affected = target.get_organ(target_zone)
	user.visible_message("<span class='notice'>[user] sews \the [target]'s [affected.name] with \the [tool].</span>", \
	"<span class='notice'>You sew \the [target]'s [affected.name] with \the [tool].</span>")
	affected.open = 0
