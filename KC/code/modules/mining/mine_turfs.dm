...
/turf/simulated/mineral //wall piece
	...
	opacity = 1
	density = 1
	var/mined_turf = /turf/simulated/floor/jungle/king
	var/ore/mineral
	var/mined_ore = 0
	var/last_act = 0
...
/turf/simulated/mineral/update_icon(var/update_neighbors)
	...
	for(var/direction in step_overlays)
		...
		if(update_neighbors && istype(turf_to_check,/turf/simulated/floor/jungle/king))
			var/turf/simulated/floor/jungle/king/T = turf_to_check
...
/turf/simulated/mineral/proc/GetDrilled(var/artifact_fail = 0)
	...
	if(artifact_find && artifact_fail)
		...
		for(var/mob/living/M in range(src, 1))
			if(pain)
				...
				if(prob(1))
					M.Stun(0)
		radiation_repository.flat_radiate(src, 25, 1)
	...
	var/turf/simulated/floor/jungle/king/N = ChangeTurf(mined_turf)
...
/turf/simulated/mineral/proc/excavate_find(var/prob_clean = 0, var/datum/find/F)
	...
	if(prob(F.prob_delicate))
		...
		if(!S)
			if(X)
				visible_message("<span class='danger'>[pick("The rock crumbles away into dust","The rock breaks apart")].</span>")
				qdel(X)
...
/turf/simulated/mineral/proc/artifact_debris(var/severity = 0)
	...
	for(var/j in 1 to rand(1, 3 + max(min(severity, 1), 0) * 2))
		switch(rand(1,5))
			if(1)
				var/obj/item/stack/material/sandstone/R = new(src)
				R.amount = rand(5,25)
...
/turf/simulated/mineral/random
	...
	var/mineralSpawnChanceList = list("Iron" = 45, "Carbon" = 30, "Diamond" = 1, "Gold" = 5, "Silver" = 5)
...
/turf/simulated/mineral/random/high_chance
	...
	mineralSpawnChanceList = list("Iron" = 45, "Carbon" = 30, "Diamond" = 4, "Gold" = 8, "Silver" = 10)
...
/turf/simulated/floor/jungle/king/Destroy()
	mining_floors -= src
	return ..()

/turf/simulated/floor/jungle/king/ex_act(severity)
	switch(severity)
		if(3.0)
			return
		if(2.0)
...
/turf/simulated/floor/jungle/king/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(!W || !user)
		return 0
...
/turf/simulated/floor/jungle/king/proc/gets_dug()
...
/turf/simulated/floor/jungle/king/proc/updateMineralOverlays(var/update_neighbors)
	...
	if(update_neighbors)
		...
		for(var/direction in all_step_directions)
			var/turf/simulated/floor/jungle/king/A
			if(istype(get_step(src, direction), /turf/simulated/floor/jungle/king))
				A = get_step(src, direction)
				A.updateMineralOverlays()

/turf/simulated/floor/jungle/king/Entered(atom/movable/M as mob|obj)
	..()
