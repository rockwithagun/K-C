...
/*
//Step one - dehairing.
/obj/item/stack/material/wetleather/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(	istype(W, /obj/item/weapon/material/knife) || \
		...
		if(do_after(user,50))
			...
			for(var/obj/item/stack/material/leather/HS in usr.loc)
				if(HS.amount < 50)
					HS.amount++
					src.use(1)
					break
			//If it gets to here it means it did not find a suitable stack on the tile.
			var/obj/item/stack/material/leather/HS = new(usr.loc)
			HS.amount = 1
			src.use(1)
	else
		..()
