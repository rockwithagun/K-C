/obj/machinery/seed_extractor
	name = "seed extractor"
	desc = "Grinds produce into seeds."
...
obj/machinery/seed_extractor/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	if(istype(O, /obj/item/weapon/reagent_containers/food/snacks/grown) || istype(O, /obj/item/weapon/grown))
		...
		if(new_seed_type)
			to_chat(user, "<span class='notice'>You scoop some seeds from [O].</span>")
	...
	//Grass.
		...
		if (S.use(1))
			to_chat(user, "<span class='notice'>You scoop some seeds from the grass tile.</span>")
