...
/datum/controller/occupations
	...
	proc/GiveRandomJob(var/mob/new_player/player)
		...
		for(var/datum/job/job in shuffle(occupations))
			...
			if(istype(job, GetJob("Peasant"))) // We don't want to give him assistant, that's boring!
				continue
 ...
 **/
	...
	proc/EquipRank(var/mob/living/carbon/human/H, var/rank, var/joined_late = 0)
		...
		if(job.req_admin_notify)
			to_chat(H, "<b>You are playing a job that is important for Game Progression. Don't die.</b>")

		if(job.slayer)
			to_chat(H, "<b>You live to hunt the creatures of the night. However, your foremost prey are the vampires that plague this land. Don't die.</b>")
