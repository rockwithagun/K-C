...
datum/objective/brig
	...
	find_target()
		..()
		if(target && target.current)
			explanation_text = "Have [target.current.real_name], the [target.assigned_role] arrested for 10 minutes."
		else
			explanation_text = "Free Objective"
		return target
	...
	find_target_by_role(role, role_type=0)
		..(role, role_type)
		if(target && target.current)
			explanation_text = "Have [target.current.real_name], the [!role_type ? target.assigned_role : target.special_role] arrested for 10 minutes."
		else
			explanation_text = "Free Objective"
		return target
...
datum/objective/steal
	...
	var/global/possible_items[] = list(
		"a gold crown" = /obj/item/clothing/head/helmet/king/crown,
		"a king's helmet" = /obj/item/clothing/head/helmet/king/heavy,
	)
